  ...
  data.map((x, y) => {
    return (cards.innerHTML += `<div id=${y} class="card">
        <span class="card__title" data-open="card-modal-list" onclick="viewCardDetail(this)">${x.name}</a></span>
          ...
          <i class="fa-sharp fa-solid fa-circle-plus" id="addbtn10
          " style="font-size:25px;margin-bottom: 10px;color: #008bff;"></i>
          </div>
          <div class="delete-button" onclick="deleteCard(this)">
          <i class="fa-solid fa-trash" style="font-size:25px;     margin-bottom: 10px; color:red;"></i>
          </div>
      </div>`);
  });
...
const addItems = () => {
  ...
  li.innerHTML = `${itemName}<button class="btn-done" onclick="itemChecked(this)">Mark done</button>`;
};
